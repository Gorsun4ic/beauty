(() => {
    ...
    mask(".callback-form__tel", "+380 (__) ___  __ __");
    ...
    const popupForm = document.querySelector(".popup-form"), popupThanks = document.querySelector(".popup-thanks"), generalPopup = document.querySelector(".popup-general");
    popupForm.addEventListener("submit", (e => {
        popupForm.style.display = "none";
        popupThanks.style.display = "block";
        e.preventDefault();
        setTimeout((() => {
            generalPopup.classList.remove("popup_show");
            document.querySelector("html").classList.remove("popup-show");
        }), "5000");
    }));
    function checkNumber(number) {
        for (let i = 0; i <= number; i++) if (i % 2 === 1) console.log(`${i} Подходит`); else console.log(`${i} Не подходит`);
    }
    checkNumber(15);
    window["FLS"] = true;
    isWebp();
